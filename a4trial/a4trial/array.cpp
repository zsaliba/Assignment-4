...
int merge(int *arrs[], int arrs_length[], int *re_arr, int & re_size){
	int k = 5;
	//min priority queue of nodes
	priority_queue <Node*, vector <Node*>, compareNode> pq;
	
	//create nodes out of first element of each array and push nodes to initialize heap
	for (int i = 0; i < k; i++) {
	...
	//putting root of pq into output array then popping root again needs to be fixed
	//to incorporate pushing & popping nodes
	int outputindex = 0;
	while (!pq.empty()) {
